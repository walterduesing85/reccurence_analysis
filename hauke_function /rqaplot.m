...
%             r_win_w(19,j) = -log((sum(R(:,j))/size(R,1)));
    ...
    if windowshape ~=0
        ...
        for startwert = 1:ws_new:length(R)-w_new+1 % Spalten-Startwert(e) f�r RP-Bildausschnitt 
                                                    % setzen.

            RP_Ausschnitt=zeros(w_new,w_new);  % leere Matrix mit Kantenl�nge w machen. 
                                               % Hierein soll nun der RP-Ausschnitt
                                               % kommen. Danach wird diese f�r die RQA
                                               % verwendet.
            ...
            for j = startwert:startwert+w_new-1     % Spalten w�hlen
                % Zeilen w�hlen
                ...
                j2=j2+1;                % Spaltenindex des RP-Ausschnitts erh�hen
            end
            % RQA machen
            r_win2(1:18,cnt2)=rqa2(RP_Ausschnitt,L,T);


            % Spaltencounter der RQA's erh�hen
            cnt2=cnt2+1;
...
elseif runningwindow == 1
    ...
    elseif windowshape ~= 0
        ...
        for i = 1:ws_new:lenTS-(w_new+timespan_diff),waitbar(i/(lenTS-(w_new+timespan_diff)))
            ...
            j2=1;                                   % Spaltenindex des RP_Auschnitt
            for j = 1:w_new-1     % Spalten w�hlen
                % Zeilen w�hlen
                i2=round(w_new/2)-counter;          % Zeilenindex des RP_Ausschnitt
                for k = (round(w_new/2)-counter):(round(w_new/2)+counter)
                    RP_Ausschnitt(i2,j2)=R(k,j);    % Werte �bernehmen
                    i2=i2+1;                        % Zeilenindex des RP_Ausschnitt anpassen
                end
